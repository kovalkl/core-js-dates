...
function getNextFriday(date) {
  const daysToFriday =
    date.getDay() < 5 ? 5 - date.getDay() : 12 - date.getDay();

  return new Date(date.getTime() + daysToFriday * 24 * 60 * 60 * 1000);
}
...
function formatDate(date) {
  ...
  const month = parseDate.getUTCMonth() + 1;
  const day = parseDate.getUTCDate();
  let hour = parseDate.getUTCHours();
  const minute = parseDate.getUTCMinutes().toString().padStart(2, '0');
  const second = parseDate.getUTCSeconds().toString().padStart(2, '0');
  let period = 'AM';
...
function getCountWeekendsInMonth(month, year) {
  ...
  const daysNumbers = new Date(year, month).getUTCDate();

  if (daysNumbers <= 28) {
    return baseWeekends;
  }

  const extraDays = daysNumbers - 28;

  for (let i = 1; i <= extraDays; i += 1) {
    const nextDay = new Date(year, month - 1, 28 + i).getUTCDay();
...
function getWeekNumberByDate(date) {
  ...
  const firstWeekDay = new Date(year, 0).getDay();
  ...
  return Math.floor(days / 7) + extraWeeks;
}
